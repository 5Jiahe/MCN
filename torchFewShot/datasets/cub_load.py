...
def load_images(data_file):
    ...
    # l = len(data['targets'])
    # if l== 2953:
    #     d = np.zeros([2953,84,84,3])
    #     a=data['features'][0]
    #     for i in range(l):
    #       noise = np.random.normal(0, 10 ** 0.5, a.shape)
    #       d[i] = data['features'][i]+noise
    #     fields = d,data['targets']
    #     return fields
    fields = data['features'], data['targets']
    return fields
...
class cub_load(object):
    ...
    """
    dataset_dir = '/home/lyh/.local/MAP-Net-main (copy)/home/abc/Datasets/cub/CUB_200_2011/npz/'
